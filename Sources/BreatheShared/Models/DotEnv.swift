...
public struct DotEnv {
    ...
    public init(fileAt path: String = "\(FileManager.default.currentDirectoryPath)/.env") {
        if ProcessInfo.processInfo.environment["XCTestConfigurationFilePath"] != nil {
            var file = (#file)
            file.removeLast(38) // Sources/FumeShared/Models/DotEnv.swift
            file.append(".env")
            self.filePath = DotEnv.buildFileUrl(from: file)
        } else {
            self.filePath = DotEnv.buildFileUrl(from: path)
        }
        load(path)
    }
    
    private static func buildFileUrl(from path: String) -> String {
